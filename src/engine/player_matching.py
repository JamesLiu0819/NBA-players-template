...
# 2026-09-11 實測校準過兩次。第一次直接對每軸取 uniform(0,100) 抽樣,結果
# 距離普遍偏大、門檻抓太高,害幾乎所有人都變 5 星,因為這跟「隨機填答」按鈕
# 的實際行為不一樣：按鈕是每軸 4 題各自獨立抽 1-5,取平均後才換算成 0-100
# 座標,分數會往中間(50)集中,分佈比直接 uniform(0,100) 窄很多。第二次改
# 用正確的抽樣方式(四軸座標＝4 個獨立 1-5 隨機值取平均;身材數值沿用網站
# randomBodyValues() 的推導邏輯,不是獨立均勻抽樣)重跑 20 次,每次取
# rank_similar_players_by_style_and_body 的前 10 名距離,合併成 200 筆樣本
# 由小到大排序,取第 40/80/120/160 名的距離(57.92/68.32/74.25/79.37,
# 四捨五入成 58/68/74/79)當門檻,把 5 個星級平均切成五等分。兩個排序函數
# 共用同一組門檻。之後如果覺得星等分佈還是不合理,可以重跑同樣的抽樣流程
# 重新校準)。
