...
# 2026-09-11 改成實測校準：用「隨機填答」按鈕同樣的全隨機方式(每軸、每項
# 身材數值都獨立均勻隨機)跑 20 次,每次取 rank_similar_players_by_style_
# and_body 的前 10 名距離,合併成 200 筆樣本,取 20/40/60/80 百分位數
# (72.88/81.09/93.94/107.63,四捨五入成 73/81/94/108)當門檻,把 5 個星
# 級平均切成五等分,不再是憑感覺猜的數字。兩個排序函數共用同一組門檻。之後
# 如果覺得星等分佈還是不合理,可以重跑同樣的抽樣流程重新校準)。
